...
- **Grilling** (HITL): Conversation. The default case. Always run the grilling interview and load the `domain-modeling` skill.
...
1. **Name the destination.** Run the grilling interview and load the `domain-modeling` skill, to pin down what this map is finding its way to: the spec, decision, or change. The destination fixes the scope, so it's settled first.
...
5. **Fire the research subagents.** For each `research` ticket you just created, spin up a subagent that loads the `research` skill to resolve it in parallel, capturing its findings on a throwaway `research/<name>` branch with a context pointer from the ticket.
...
3. Resolve it. **Zoom as needed**: fetch the full body of any related or closed ticket on demand; load whichever skills the `## Notes` block names. If in doubt, run the grilling interview and load the `domain-modeling` skill.
