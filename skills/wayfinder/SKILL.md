...
- **Grilling** (HITL): Conversation. The default case. Always load the `grilling` and `domain-modeling` skills.
...
1. **Name the destination.** Load the `grilling` and `domain-modeling` skills, to pin down what this map is finding its way to: the spec, decision, or change. The destination fixes the scope, so it's settled first.
...
3. Resolve it. **Zoom as needed**: fetch the full body of any related or closed ticket on demand; load whichever skills the `## Notes` block names. If in doubt, load the `grilling` and `domain-modeling` skills.
