...
1. **`/grill-with-docs`** sharpens the idea by interview. It's stateful, retaining what it learns in `CONTEXT.md` and ADRs.
2. **Branch: can you settle every question in conversation?** If a question needs a runnable answer (state, business logic, a UI you have to see), detour through a prototype (see Phase boundaries):
   - **`/prototype`** to answer the question with throwaway code in its own directory,
   - come back and fold what you learned into the idea thread.
...
- **`/grilling`** is the interview primitive itself: rounds, the frontier, facts are the agent's job and decisions are yours. `/grill-with-docs` is the named way in, and `/triage`, `/wayfinder` and `/improve-codebase-architecture` all run it internally. Reach for it directly only when you want the interview with no wrapper around it.
